...
Milestone 14: DQN with multi-step returns.
"""
...
def train(
    ...
    print(f"Starting n-step DQN training for {total_steps} steps...")
    ...
    while step < total_steps:
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"nstep_dqn_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    final_checkpoint = str(checkpoint_dir / "nstep_dqn_final.pt")
...
def evaluate(
    ...
    num_games: int,
    agent: Optional[NStepDQNAgent] = None
) -> EvalResult:
    """Evaluate n-step DQN agent. Per DEC-0006."""
    env = env_factory()
    device = env.device
    ...
    if agent is None:
        ...
        agent = NStepDQNAgent(device=device, hidden_layers=hidden_layers, n_steps=n_steps)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="n-step DQN Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/nstep_dqn/config.yaml",
