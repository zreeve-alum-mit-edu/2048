...
Milestone 12: Dueling DQN with separate value and advantage streams.
"""
...
def train(
    ...
    print(f"Starting Dueling DQN training for {total_steps} steps...")
    ...
    while step < total_steps:
        ...
        # Recompute valid mask for reset states
        if result.done.any():
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"dueling_dqn_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    final_checkpoint = str(checkpoint_dir / "dueling_dqn_final.pt")
...
def evaluate(
    ...
    num_games: int,
    agent: Optional[DuelingDQNAgent] = None
) -> EvalResult:
    """Evaluate Dueling DQN agent. Per DEC-0006."""
    env = env_factory()
    device = env.device
    ...
    if agent is None:
        ...
        agent = DuelingDQNAgent(device=device, hidden_layers=hidden_layers)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Dueling DQN Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/dueling_dqn/config.yaml",
