"""
IMPALA (Importance Weighted Actor-Learner Architecture) Entry Point.
...
def train(
    ...
    agent = IMPALAAgent(
        ...
        n_steps=config["impala"]["n_steps"],
        rho_bar=config["impala"]["rho_bar"],
        c_bar=config["impala"]["c_bar"],
    ...
    while step < total_steps:
        for _ in range(n_steps):
            ...
            # Get behavior policy logits before action selection
            behavior_logits = agent.get_behavior_logits(state, valid_mask)

            # Select action
            actions = agent.select_action(state, valid_mask, training=True)

            # Store transition with behavior logits
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"impala_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    final_checkpoint = str(checkpoint_dir / "impala_final.pt")
...
def evaluate(
    ...
    num_games: int,
    agent: Optional[IMPALAAgent] = None
) -> EvalResult:
    """Evaluate IMPALA agent.
    ...
    if agent is None:
        ...
        agent = IMPALAAgent(device=device, hidden_layers=hidden_layers)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="IMPALA Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/impala/config.yaml",
