"""
PER DQN (Prioritized Experience Replay) Entry Point.
...
Milestone 13: DQN with prioritized experience replay.
"""
...
def train(
    ...
    print(f"Starting PER DQN training for {total_steps} steps...")
    ...
    while step < total_steps:
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"per_dqn_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    final_checkpoint = str(checkpoint_dir / "per_dqn_final.pt")
...
def evaluate(
    ...
    num_games: int,
    agent: Optional[PERDQNAgent] = None
) -> EvalResult:
    """Evaluate PER DQN agent. Per DEC-0006."""
    env = env_factory()
    device = env.device
    ...
    if agent is None:
        ...
        agent = PERDQNAgent(device=device, hidden_layers=hidden_layers)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="PER DQN Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/per_dqn/config.yaml",
