"""
ACER (Actor-Critic with Experience Replay) Algorithm Entry Point.
...
def train(
    ...
    agent = ACERAgent(
        ...
        c=config["acer"]["c"],
        replay_ratio=config["acer"]["replay_ratio"],
        buffer_capacity=config["acer"]["buffer_capacity"],
        buffer_min_size=config["acer"]["buffer_min_size"],
        batch_size=config["acer"]["batch_size"],
    ...
    print(f"Starting ACER training for {total_steps} steps...")
    ...
    while step < total_steps:
        for _ in range(n_steps):
            ...
            actions, mu = agent.select_action(state, valid_mask, training=True)

            # Store with placeholder reward/done
        ...
        if train_metrics is not None:
            ...
            metrics["q_losses"].append(train_metrics["q_loss"])
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"acer_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    final_checkpoint = str(checkpoint_dir / "acer_final.pt")
...
def evaluate(
    ...
    if agent is None:
        ...
        agent = ACERAgent(device=device, hidden_layers=hidden_layers)
    ...
    while games_completed < num_games:
        ...
        while not done_mask.all():
            ...
            actions, _ = agent.select_action(state, valid_mask, training=False)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="ACER Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/acer/config.yaml",
