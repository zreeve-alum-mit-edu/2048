"""
SARSA(lambda) Algorithm Entry Point with Eligibility Traces.
...
bridging TD(0) and Monte Carlo methods.
"""
...
def train(
    ...
    """Train SARSA(lambda) agent.

    Per DEC-0006: Required interface for algorithm training.

    SARSA(lambda) maintains eligibility traces across steps to
    propagate TD errors to previously visited states.
    """
    ...
    # Get initial valid mask and action
    ...
    print(f"Starting SARSA(lambda) training for {total_steps} steps (lambda={config['sarsa_lambda']['lambda']})...")
    ...
    while step < total_steps:
        ...
        # Select next action
        next_action = agent.select_action(next_state, next_valid_mask, training=True)

        # SARSA(lambda) update with traces
        ...
        metrics["td_errors"].append(train_metrics["td_error"])
        metrics["trace_norms"].append(train_metrics["trace_norm"])
        ...
        if result.done.any():
            ...
            # (Agent handles this internally based on done mask)
        ...
        if step % log_frequency == 0:
            avg_loss = sum(metrics["losses"][-log_frequency:]) / log_frequency
            avg_q = sum(metrics["q_means"][-log_frequency:]) / log_frequency
            avg_trace = sum(metrics["trace_norms"][-log_frequency:]) / log_frequency
            eps = agent.epsilon
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"sarsa_lambda_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    final_checkpoint = str(checkpoint_dir / "sarsa_lambda_final.pt")
...
def evaluate(
    ...
    num_games: int,
    agent: Optional[SARSALambdaAgent] = None
) -> EvalResult:
    """Evaluate SARSA(lambda) agent. Per DEC-0006."""
    env = env_factory()
    device = env.device
    ...
    if agent is None:
        ...
        lambda_ = checkpoint.get("lambda", 0.9)
        agent = SARSALambdaAgent(
            device=device,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="SARSA(lambda) Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/sarsa_lambda/config.yaml",
