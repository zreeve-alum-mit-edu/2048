"""
A2C (Advantage Actor-Critic) Algorithm Entry Point.
...
This module implements the required interface for A2C training and evaluation.
A2C is a synchronous actor-critic algorithm that uses n-step returns and
...
def train(
    ...
    A2C collects n-step rollouts and updates using advantage estimation
    ...
    print(f"Starting A2C training for {total_steps} steps...")
    ...
    while step < total_steps:
        ...
        for _ in range(n_steps):
            ...
            agent.store_transition(
                state=state,
                action=actions,
                reward=torch.zeros(env.n_games, device=device),  # Will be filled after step
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"a2c_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    # Save final checkpoint
    final_checkpoint = str(checkpoint_dir / "a2c_final.pt")
...
def evaluate(
    ...
    if agent is None:
        ...
        agent = A2CAgent(device=device, hidden_layers=hidden_layers)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="A2C Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/a2c/config.yaml",
