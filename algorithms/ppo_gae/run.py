"""
PPO+GAE (Proximal Policy Optimization with Generalized Advantage Estimation) Entry Point.
...
Milestone 10: PPO with Generalized Advantage Estimation - builds on A2C's
actor-critic structure with clipped surrogate objective and multiple epochs.
"""
...
def train(
    ...
    PPO collects rollouts and performs multiple epochs of minibatch updates
    using the clipped surrogate objective and GAE for advantage estimation.
    ...
    print(f"Starting PPO+GAE training for {total_steps} steps...")
    ...
    while step < total_steps:
        ...
        for _ in range(n_steps):
            ...
            # Select action with log prob and value
            actions, log_probs, values = agent.select_action_with_log_prob(state, valid_mask)

            # Environment step
            result = env.step(actions)

            # Store transition (reward will be the actual reward)
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"ppo_gae_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    # Save final checkpoint
    final_checkpoint = str(checkpoint_dir / "ppo_gae_final.pt")
...
def evaluate(
    ...
    if agent is None:
        ...
        agent = PPOAgent(device=device, hidden_layers=hidden_layers)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="PPO+GAE Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/ppo_gae/config.yaml",
