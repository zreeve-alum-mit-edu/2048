"""
Double DQN Algorithm Entry Point.
...
This module implements the required interface for Double DQN training and evaluation.
Double DQN reduces overestimation bias by using the policy network to select actions
...
def train(
    ...
    print(f"Starting Double DQN training for {total_steps} steps...")
    ...
    while step < total_steps:
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"double_dqn_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    # Save final checkpoint
    final_checkpoint = str(checkpoint_dir / "double_dqn_final.pt")
...
def evaluate(
    ...
    if agent is None:
        ...
        agent = DoubleDQNAgent(device=device, hidden_layers=hidden_layers)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Double DQN Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/double_dqn/config.yaml",
