...
Milestone 15: Rainbow-lite = Double + Dueling + PER + n-step.
"""
...
def train(
    ...
    agent = RainbowLiteAgent(
        ...
        n_steps=config["nstep"]["n_steps"],
        per_alpha=config["per"]["alpha"],
        per_beta_start=config["per"]["beta_start"],
        per_beta_frames=config["per"]["beta_frames"],
    )
    ...
    print(f"Starting Rainbow-lite training for {total_steps} steps...")
    print("Components: Double DQN + Dueling + PER + n-step")
    ...
    while step < total_steps:
        ...
        if train_metrics is not None:
            ...
            metrics["td_errors"].append(train_metrics["td_error_mean"])
            metrics["avg_n_steps"].append(train_metrics["avg_n_steps"])
        ...
        if step % log_frequency == 0:
            ...
            avg_td = sum(metrics["td_errors"][-log_frequency:]) / log_frequency if metrics["td_errors"] else 0
            avg_n = sum(metrics["avg_n_steps"][-log_frequency:]) / log_frequency if metrics["avg_n_steps"] else 0
            eps = agent.epsilon
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"rainbow_lite_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    final_checkpoint = str(checkpoint_dir / "rainbow_lite_final.pt")
...
def evaluate(
    ...
    num_games: int,
    agent: Optional[RainbowLiteAgent] = None
) -> EvalResult:
    """Evaluate Rainbow-lite agent. Per DEC-0006."""
    env = env_factory()
    device = env.device
    ...
    if agent is None:
        ...
        agent = RainbowLiteAgent(device=device, hidden_layers=hidden_layers, n_steps=n_steps)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Rainbow-lite Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/rainbow_lite/config.yaml",
