"""
SARSA (State-Action-Reward-State-Action) Algorithm Entry Point.
...
SARSA is an on-policy TD control algorithm that uses the actual next
action for bootstrapping, making it account for the exploration policy.
"""
...
def train(
    ...
    SARSA requires knowing the next action before updating, so we use
    a two-step lookahead: select action, step, select next action, update.
    ...
    print(f"Starting SARSA training for {total_steps} steps...")
    ...
    while step < total_steps:
        ...
        # Get next valid mask
        # For done games, compute mask for reset_states
        if result.done.any():
        ...
        # Update state for next iteration (handle episode boundaries per DEC-0003)
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"sarsa_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    final_checkpoint = str(checkpoint_dir / "sarsa_final.pt")
...
def evaluate(
    ...
    num_games: int,
    agent: Optional[SARSAAgent] = None
) -> EvalResult:
    ...
    if agent is None:
        ...
        agent = SARSAAgent(device=device, hidden_layers=hidden_layers)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="SARSA Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/sarsa/config.yaml",
