"""
Expected SARSA Algorithm Entry Point.
...
Expected SARSA uses the expected value of Q under the policy for
...
def train(
    ...
    print(f"Starting Expected SARSA training for {total_steps} steps...")
    ...
    while step < total_steps:
        ...
        # Expected SARSA update (no need for next_action)
        ...
        metrics["expected_q_means"].append(train_metrics["expected_q_mean"])
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"expected_sarsa_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    final_checkpoint = str(checkpoint_dir / "expected_sarsa_final.pt")
...
def evaluate(
    ...
    num_games: int,
    agent: Optional[ExpectedSARSAAgent] = None
) -> EvalResult:
    """Evaluate Expected SARSA agent. Per DEC-0006."""
    env = env_factory()
    device = env.device
    ...
    if agent is None:
        ...
        agent = ExpectedSARSAAgent(device=device, hidden_layers=hidden_layers)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Expected SARSA Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/expected_sarsa/config.yaml",
