...
Milestone 22: C51 (Categorical/Distributional DQN).
"""
...
def train(
    ...
    agent = C51Agent(
        ...
        n_atoms=config["distributional"]["n_atoms"],
        v_min=config["distributional"]["v_min"],
        v_max=config["distributional"]["v_max"],
    )
    ...
    print(f"Starting C51 training for {total_steps} steps...")
    print(f"Distribution: {config['distributional']['n_atoms']} atoms, "
          f"V_min={config['distributional']['v_min']}, V_max={config['distributional']['v_max']}")
    ...
    while step < total_steps:
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"c51_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    final_checkpoint = str(checkpoint_dir / "c51_final.pt")
...
def evaluate(
    ...
    """Evaluate C51 agent. Per DEC-0006."""
    env = env_factory()
    device = env.device
    ...
    if agent is None:
        ...
        n_atoms = checkpoint.get("n_atoms", 51)
        v_min = checkpoint.get("v_min", 0.0)
        v_max = checkpoint.get("v_max", 100000.0)
        agent = C51Agent(
            device=device,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="C51 Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/c51/config.yaml",
