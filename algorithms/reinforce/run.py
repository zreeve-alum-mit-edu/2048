"""
REINFORCE Algorithm Entry Point.
...
This module implements the required interface for REINFORCE training and evaluation.
REINFORCE is a vanilla policy gradient algorithm that uses Monte Carlo returns
...
def train(
    ...
    REINFORCE collects complete trajectories and updates the policy
    ...
    while step < total_steps:
        ...
        # Select actions (stochastic during training)
        ...
        if num_complete >= update_frequency:
            train_metrics = agent.train_step(min_trajectories=1)

            if train_metrics is not None:
                metrics["losses"].append(train_metrics["loss"])
                metrics["avg_returns"].append(train_metrics["avg_return"])
        ...
        if step % log_frequency == 0:
            ...
            avg_return = sum(metrics["avg_returns"][-100:]) / len(metrics["avg_returns"][-100:]) if metrics["avg_returns"] else 0
        ...
        if step % checkpoint_frequency == 0:
            checkpoint_path = str(checkpoint_dir / f"reinforce_step_{step}.pt")
            agent.save_checkpoint(checkpoint_path)
            checkpoints.append(checkpoint_path)
            print(f"  Saved checkpoint: {checkpoint_path}")

    # Save final checkpoint
    final_checkpoint = str(checkpoint_dir / "reinforce_final.pt")
...
def evaluate(
    ...
    num_games: int,
    agent: Optional[REINFORCEAgent] = None
) -> EvalResult:
    ...
    if agent is None:
        ...
        agent = REINFORCEAgent(device=device, hidden_layers=hidden_layers)
    ...
    while games_completed < num_games:
        ...
        while not done_mask.all():
            ...
            # Greedy action selection (no sampling for evaluation)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="REINFORCE Training and Evaluation")
    parser.add_argument("mode", choices=["train", "evaluate"], help="Mode to run")
    parser.add_argument("--config", default="algorithms/reinforce/config.yaml",
